...
## Decisión
1. **Una sola base de datos PostgreSQL compartida** entre las dos aplicaciones backend, definida una única vez en `php-app/sql/schema.sql` y montada como script de inicialización del contenedor `postgresql_db`. Ni PHP ni Spring Boot generan o migran el esquema por sí mismos (no se usó `php artisan migrate` ni Hibernate `ddl-auto=update`), precisamente para que ambas tecnologías lean/escriban la misma estructura sin divergencias.
